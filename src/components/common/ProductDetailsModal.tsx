...
const ProductDetailsModal: React.FC<ProductDetailsModalProps> = ({ visible, onClose, product }) => {
  if (!product) return null;

  return (
    <Modal
      title={<Title level={3} style={{ marginBottom: 0 }}>🛍️ Product Details</Title>}
      open={visible}
      onCancel={onClose}
      footer={null}
      width={850}
    ...
    >
      <Descriptions bordered column={2} size="middle" layout="vertical">
        ...
        <Descriptions.Item label={<Text strong>Brand</Text>}>
          ...
          <br />
          <Image src={product.productBrand?.image} alt="Brand" width={50} height={50} style={{ borderRadius: 5 }} />
        </Descriptions.Item>
        ...
        <Descriptions.Item label={<Text strong>Stock</Text>}>
          <Tag color={product.productStock > 0 ? "green" : "red"}>{product.productStock}</Tag>
        </Descriptions.Item>
        ...
        <Descriptions.Item label={<Text strong>Featured</Text>}>
          {product.isFeatured ? <Tag color="green">Yes</Tag> : <Tag color="red">No</Tag>}
        </Descriptions.Item>
      </Descriptions>
      ...
      <Image src={product.productFeatureImage} alt="Feature" width={250} height={250} style={{ borderRadius: 10, boxShadow: "0px 4px 8px rgba(0, 0, 0, 0.2)" }} />
      
      <Divider dashed />
      
      <Title level={5}>🖼️ Other Images</Title>
      {product.productImages?.map((img, index) => (
        <Image key={index} src={img} alt={`Product-${index}`} width={100} height={100} style={{ marginRight: 8, borderRadius: 8, boxShadow: "0px 2px 6px rgba(0, 0, 0, 0.2)" }} />
      ))}
      ...
      {product.variantcolor && product.variantcolor.length > 0 && (
        <>
          ...
          {product.variantcolor.map((color) => (
            <Tag key={color._id} color={color.colorCode} style={{ padding: "5px 10px", fontSize: "14px" }}>
              {color.name}
            </Tag>
          ))}
        </>
