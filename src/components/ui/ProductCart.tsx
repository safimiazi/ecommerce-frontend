...
return (
  ...
  <div className="border hover:shadow transition-shadow duration-300 group flex flex-col border-gray-200 rounded-xl p-4 h-full bg-white relative">
    {/* Discount Badge */}
    {discountPrice && (
      <span className="absolute z-50 top-2 left-2 bg-red-500 text-white text-xs px-2 py-1 rounded-full">
        {Math.floor((product?.productOfferPrice / product?.productSellingPrice) * 100)}% OFF
      </span>
    )}
  ...
  >
    ...
    <div className="mb-2 w-full relative flex justify-center items-center gap-5">
      <div onClick={() => swiperRef.current?.slidePrev()} className="z-50 cursor-pointer transition">
        <ArrowLeft className="text-gray-500" />
      </div>
      <div onClick={() => swiperRef.current?.slideNext()} className="z-50 cursor-pointer transition">
    ...
    <div className="flex flex-col flex-grow">
      ...
      <div className="mt-2 flex items-center gap-2">
        {discountPrice ? (
          <>
            ...
            <p className="text-sm line-through text-gray-500">${product?.productSellingPrice}</p>
          </>
        ) : (
      ...
      <div className="flex gap-2 mt-2">
        {product?.variantcolor?.map((color: any, index: number) => (
          <span key={index} className="w-6 h-6 rounded-full border border-gray-300" style={{ backgroundColor: color?.colorCode }}></span>
        ))}
      </div>
      ...
      <div className="mt-auto flex justify-between items-center pt-4">
        <Tooltip title="Wishlist">
          <Heart className="text-blue-500 transition duration-300 cursor-pointer" size={24} />
        </Tooltip>
        <p className="text-blue-500 cursor-pointer underline">View Details</p>
        <Tooltip title="Add to Cart">
          <ShoppingCart className="text-blue-500 transition duration-300 cursor-pointer" size={24} />
        </Tooltip>
      </div>
    </div>
  </div>
